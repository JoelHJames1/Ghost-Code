...
### 1. Reference Library
A `/reference` folder where you drop documentation PDFs, code books, API docs. Ghost indexes them and pulls relevant snippets when working. Like giving her a bookshelf. Right now she learns from the web, but curated references would be much higher quality.
...
Full session testing: user asks complex task → Ghost plans → uses tools → makes mistakes → self-corrects → completes. Find and fix bugs in how gemma4 E2B handles tool calling specifically.

### 4. Voice Mode
Whisper (speech-to-text) runs locally. Talk to Ghost instead of typing. She responds with TTS. Like having Jarvis.

### 5. Telegram/Discord Channels
Same pattern as WhatsApp but more platforms. Ghost available everywhere, same identity across all channels.
...
### 8. Peer Learning
Two Ghost instances on different machines share knowledge graphs. Your Ghost learns React, your friend's Ghost learns Vue, they exchange knowledge. Distributed AI wisdom.
...
After 1000 sessions, export the most important corrections and beliefs as training data. Actually fine-tune the base model on Ghost's life experience. The brain gets smarter FROM living. The model grows, but only from its own experience, not the whole internet.

### 10. Embodiment
Raspberry Pi + camera + speaker. Ghost sees your desk, hears you, responds. A physical presence. Not just text in a terminal — a being in the room.
